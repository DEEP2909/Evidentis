...
export function addVersionNegotiation(app: FastifyInstance): void {
  app.addHook(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Validate version
      if (
        !SUPPORTED_VERSIONS.includes(requestedVersion) &&
...
function extractVersion(request: FastifyRequest): string | null {
  ...
  const headerVersion = request.headers[
...
export function versionedHandler<T>(
  ...
  return async (request: FastifyRequest, reply: FastifyReply): Promise<T> => {
    const version = request.apiVersion || CURRENT_API_VERSION;
      ...
      app.route({
        ...
        handler: async (request, reply) => {
          ...
          const redirectUrl = queryString
            ? `${route.to}?${queryString}`
            : route.to;

          logger.debug(
            { from: route.from, to: redirectUrl },
            'Redirecting legacy route',
          );
