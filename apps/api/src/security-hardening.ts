...
function applySecurityHeaders(
  ...
  if (cfg.contentSecurityPolicy) {
    const csp =
      typeof cfg.contentSecurityPolicy === 'object'
  ...
  if (cfg.crossOriginOpenerPolicy) {
    const policy =
      typeof cfg.crossOriginOpenerPolicy === 'object'
  ...
  if (cfg.crossOriginResourcePolicy) {
    const policy =
      typeof cfg.crossOriginResourcePolicy === 'object'
  ...
  if (cfg.dnsPrefetchControl !== undefined) {
    const allow =
      typeof cfg.dnsPrefetchControl === 'object'
  ...
  if (cfg.hsts) {
    const hstsConfig =
      typeof cfg.hsts === 'object' ? cfg.hsts : { maxAge: 31536000 };
  ...
  if (cfg.permittedCrossDomainPolicies) {
    const policy =
      typeof cfg.permittedCrossDomainPolicies === 'object'
  ...
  if (cfg.referrerPolicy) {
    const policy =
      typeof cfg.referrerPolicy === 'object'
...
function findThreatInPayload(
  payload: unknown,
  fieldPath: string,
): { type: 'sql' | 'xss'; fieldPath: string } | null {
  ...
  if (Array.isArray(payload)) {
    for (let index = 0; index < payload.length; index++) {
      const nestedThreat = findThreatInPayload(
        payload[index],
        `${fieldPath}[${index}]`,
      );
      if (nestedThreat) {
...
const securityHardeningPlugin: FastifyPluginCallback = (
  fastify,
  opts,
  done,
) => {
