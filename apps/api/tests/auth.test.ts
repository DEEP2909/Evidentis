...
describe('JWT Access Tokens', () => {
  ...
  it('should reject tampered tokens', async () => {
    ...
    parts[1] = Buffer.from(JSON.stringify({ sub: 'hacker' })).toString(
      'base64',
    );
...
describe('POST /auth/login', () => {
  ...
  it('should lock account after 5 failed attempts', async () => {
    ...
    // Cleanup
    await pool.query(`DELETE FROM attorneys WHERE email = $1`, [lockTestEmail]);
  });
});
...
describe('Password Reset Flow', () => {
  ...
  it('should reset password with valid token', async () => {
    ...
    await pool.query(`UPDATE attorneys SET password_hash = $2 WHERE id = $1`, [
      TEST_ATTORNEY.id,
      newHash,
    ]);
  });
