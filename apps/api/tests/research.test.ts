    ...
    const flags = [
      {
        severity: 'critical',
        rule_id: 'rule1',
        description: 'Uncapped indemnity',
      },
      {
        severity: 'high',
        rule_id: 'rule2',
        description: 'Weak limitation of liability',
      },
      {
        severity: 'medium',
        rule_id: 'rule3',
        description: 'Non-compete risk in Karnataka',
      },
      {
        severity: 'low',
        rule_id: 'rule4',
        description: 'Missing notice period',
      },
    ];
...
describe('Obligation Tracking', () => {
  ...
  describe('GET /api/documents/:id/obligations', () => {
    ...
    it('should include overdue status', async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = JSON.parse(response.body);
      const overdueObl = body.find(
        (o: any) => o.title === 'Send 30-day notice',
      );
...
describe('Worker Integration', () => {
  describe('Document Processing Queue', () => {
    it('should enqueue document for processing', async () => {
      ...
      try {
        const jobId = await enqueueDocumentScan(
          documentId,
          TEST_TENANT.id,
          '/test/worker',
        );
