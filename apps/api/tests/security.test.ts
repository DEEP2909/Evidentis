...
afterAll(async () => {
  // Cleanup in order
  await pool.query(`DELETE FROM documents WHERE tenant_id IN ($1, $2)`, [
    TENANT_A.id,
    TENANT_B.id,
  ]);
  await pool.query(`DELETE FROM matters WHERE tenant_id IN ($1, $2)`, [
    TENANT_A.id,
    TENANT_B.id,
  ]);
  await pool.query(`DELETE FROM attorneys WHERE tenant_id IN ($1, $2)`, [
    TENANT_A.id,
    TENANT_B.id,
  ]);
...
describe('Tenant Isolation - CRITICAL', () => {
  describe('Matter Access', () => {
    ...
    it('Tenant B cannot delete Tenant A matter', async () => {
      ...
      const check = await pool.query(`SELECT id FROM matters WHERE id = $1`, [
        matterA,
      ]);
...
describe('SQL Injection Prevention', () => {
  it('should reject SQL injection in search parameter', async () => {
    ...
    expect(
      body.items.every(
        (m: any) => m.matterName.includes("' OR '1'='1") === false,
      ),
    ).toBe(true);
  });
  ...
  it('should reject SQL injection in ID parameter', async () => {
    ...
    expect(Number.parseInt(check.rows[0].count)).toBeGreaterThan(0);
  });
...
describe('CSRF Protection', () => {
  ...
  it('should validate token signature', async () => {
    const tamperedToken = tokenA
      .split('.')
      .map((p, i) => (i === 1 ? 'tamperedpayload' : p))
      .join('.');
